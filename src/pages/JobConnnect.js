...
const JobConnect = () => {
  ...
  const [favoritedJobIds, setFavoritedJobIds] = useState(
    () => new Set(favorites.map((job) => job.id))
  );
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      try {
        ...
        setUserName(parsedUser.username || parsedUser.fullName || "User");
      } catch (err) {
  ...
  useEffect(() => {
    ...
    if (storedPreferences) {
      try {
        const { jobTitles, jobLevel } = JSON.parse(storedPreferences);
        if (jobTitles && jobTitles.length > 0) {
          // Join all job titles into a single search query
          setSearchTerm(jobTitles.join(" OR "));
        }
        // You can also do something with jobLevel if needed
        console.log("Preferred level:", jobLevel);
      } catch (err) {
        console.error("Error parsing job preferences:", err);
      }
    }
  }, []);
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      ...
      try {
        const jobLevelParam =
          jobLevel && jobLevel !== "" ? `&category=${jobLevel}` : "";
        const response = await fetch(
          `https://api.adzuna.com/v1/api/jobs/us/search/1?app_id=${appId}&app_key=${appKey}&what=${searchTerm}${jobLevelParam}`
        );
        ...
        if (data && data.results) {
          ...
          setJobs([]);
          console.warn(
            "No job results found or invalid data format from Adzuna."
          );
        }
      } catch (error) {
              ...
              <div className="job-actions">
                <button
                  className={`favorite-button ${
                    favoritedJobIds.has(job.id) ? "favorited" : ""
                  }`}
                  onClick={() => handleAddToFavorites(job)}
                >
                  <FaHeart />{" "}
                  {favoritedJobIds.has(job.id) ? "Favorited" : "Favorite"}
                </button>
                <a
          ...
          <div className="job-details">
            ...
            <div className="job-meta-details">
              ...
              {selectedJob.contract_type && (
                <p>Contract Type: {selectedJob.contract_type}</p>
              )}
            </div>
